...
const ButtonsBlock = (props:Props) => {
  const { typeOfCalc, changeType } = useContext(NumberContext);
  let classListItem;
  classListItem = typeOfCalc==='Runtime' ? ' full' : ''
  return (
    <div className={`btn_container ${classListItem}`}>
      <div className={`btn btn_run ${typeOfCalc==='Runtime' ? 'active' :''}`} onClick={(e)=>changeType('Runtime')}>
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
     ...
     <div className= {`btn btn_constr ${typeOfCalc==='Constructor' ? 'active' :''}`} onClick={(e)=>changeType('Constructor')}>
     <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
      ...
      >
        <path
          d="M7.49984 13.3333L4.1665 10L7.49984 6.66668M12.4998 6.66668L15.8332 10L12.4998 13.3333"
          stroke="#4D5562"
          stroke-width="2"
          stroke-linecap="round"
          stroke-linejoin="round"
